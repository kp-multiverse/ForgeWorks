...
- Resume brief (`go`): `python3 scripts/resume.py` | backlog view: `python3 scripts/backlog.py` | ONE feature entry: `python3 scripts/backlog.py --feature <id>` | factory doctor (prune stale worktrees + merged branches): `bash scripts/factory_doctor.sh` | doc prune (deletes what closed features leave behind): `python3 scripts/prune.py` (`--check` is the CI entry-cap gate and owns the caps)
