...
<project>
Chunkline -- A Rust library that splits large text files into stable, overlap-aware chunks for indexing pipelines.
Primary user: A backend developer building document indexing who needs deterministic chunk boundaries.. Stack: Rust; frontend: no; AI features: none. Dev container: no (if yes, commands run inside it).
Where things live:
