...
- **Line caps.** The ~100-line target / 200-line hard cap applies to **generated project files** -- everything under `templates/core/` and `templates/profiles/` (it becomes someone else's code). The generated `AGENTS.md` constitution is the one file with no slack: `render.py` hard-fails the render if it exceeds 100 lines, and the generated CI's `docs-budget` job re-checks the same cap on every push. Line caps do NOT apply to the generator/meta files: `init-project/SKILL.md`, `upgrade-project/SKILL.md`, and this repo's own `AGENTS.md` are reference documents (interview + emission logic) and are necessarily longer.
</editing-the-template>
