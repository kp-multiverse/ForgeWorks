...
- `init-project/SKILL.md` -- the interview skill. Runs in the target project after install. Phase 2 is a conversation, not a numbered form: at most 5 questions plus a drafted `docs/PRD.md` the owner must explicitly approve before generation. Everything else takes a documented default. It writes the answers to `docs/_init-answers.json` (schema documented in Phase 4, with top-level `features` and `design` sections -- still the render contract), runs the renderer, then installs deps (Phase 4.5, the only agent-executed generation step -- it runs package managers).
- `init-project/render.py` -- the deterministic renderer (stdlib Python, plain string substitution, no template engine). Takes `--answers/--core/--profile/--out` and executes every substitution and conditional rule: placeholder table (including `{{PRD_SURFACES}}` and `{{TEST_PATH_REGEX}}`), AI-discipline block, AI/CC fences, style references, mem0/memory block, gotchas seed, `docs/features.json` + `docs/agents.json` emission, explanations/devcontainer opt-outs, Codex conditionals, JSON/TOML escaping of hostile answers, multi-line re-indent, manifest `.example` rename, `CLAUDE.md` symlink, `chmod +x`, `.claude/.template-version` stamp, and a hard-fail if rendered `AGENTS.md` exceeds its line cap (`rule_check.py` asserts the renderer and the shipped `docs-budget` job agree on that number). Same answers, same bytes; it fails closed on invalid answers or any leftover `{{...}}`.
- `init-project/templates/core/` -- the language-free files every project gets: the AGENTS.md constitution, `docs/` (`PRD.md`, `LEDGER.md`, `BACKLOG.md`, `features.json`'s source, `docs/plans/` -- working state only, a plan is deleted at merge -- `docs/deviations.md`), security files, `.mcp.json`, CI shape (including the `features-check`, `docs-budget`, and `test-tamper` jobs), README, .env.example, `scripts/` (`backlog.py` -- with the `--feature <id>` single-entry read, `resume.py` -- the derived `go` brief, `dup_check.py`, `factory_doctor.sh`, `skills_doctor.py` -- the instruction-stack check: one copy per skill name, no always-on SessionStart plugin, the real session-start inventory the repo-side budget cannot see, `features_check.py` -- which also fails a plan left behind by a closed feature, `tamper_check.py`), the generated skills (`iteration`, `security-review`, `tech-debt`) and subagents (`reviewer`, `utility` -- there is no implementer subagent; main context drives GRILL/GREEN itself). `docs/design/` ships only when the project has a frontend (`render.py`'s `skip_file` drops it otherwise). Uses `{{PLACEHOLDER}}` substitution.
...
- `.github/scripts/rule_check.py` -- the drift gate. A gate owns its number; prose points at the gate. It renders every fixture and fails when a rule restates a cap its CI job enforces, when prose cites a path the rendered project does not have, or when `render.py`'s hard-coded `AGENTS.md` line cap disagrees with the shipped `docs-budget` job. Every defect found in the v4.1 field review was one fact written twice and drifting; this is what would have caught them without a real project.
...
- **Line caps.** The soft target of about a hundred lines, and the mechanical 200-line file cap, apply to **generated project files** -- everything under `templates/core/` and `templates/profiles/` (it becomes someone else's code). The generated `AGENTS.md` constitution is the one file with no slack: `render.py` hard-fails the render if it exceeds the cap, and the generated CI's `docs-budget` job re-checks the same number on every push -- `rule_check.py` fails if those two ever disagree. Line caps do NOT apply to the generator/meta files: `init-project/SKILL.md`, `upgrade-project/SKILL.md`, and this repo's own `AGENTS.md` are reference documents (interview + emission logic) and are necessarily longer.
</editing-the-template>
