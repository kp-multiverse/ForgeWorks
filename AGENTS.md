...
- `.github/fixtures/golden/` -- seven answers fixtures spanning the conditional matrix (AI/no-AI, devcontainer, opt-ins, hostile values) plus committed expected trees under `expected/<name>/`. The CI `golden-fixtures` job (`.github/scripts/golden_test.py`) re-renders each fixture and compares byte-for-byte; `--update` regenerates the trees after an intentional template change.
