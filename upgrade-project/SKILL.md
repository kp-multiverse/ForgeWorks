...
```bash
npx --yes degit@2.8.4 kp-multiverse/ForgeWorks/init-project/templates/core#v4.7.0 /tmp/upgrade-core --force
npx --yes degit@2.8.4 kp-multiverse/ForgeWorks/init-project/templates/profiles/<lang>#v4.7.0 /tmp/upgrade-profile --force
npx --yes degit@2.8.4 kp-multiverse/ForgeWorks/init-project#v4.7.0 /tmp/upgrade-skill --force
```
...
Reconcile against this skill's own released version (`v4.7.0`), not `main`: installing the `vX.Y.Z` upgrade skill brings a project *up to* `vX.Y.Z` -- a versioned, reviewable target. (Each release bumps this ref; see the repo `AGENTS.md` release process.)
...
*Language-independent delta (since v4.7.0) -- skill hygiene:* copy `scripts/skills_doctor.py` from the fetched template (placeholder-free, `chmod +x`) and replace `scripts/factory_doctor.sh` with the fetched version (template-owned; diff first, report any project-added lines). **Run `python3 scripts/skills_doctor.py` once and put its findings first in the Phase 4 report.** A `FAIL` is a skill name installed from more than one source (plugin + personal + project) or an enabled plugin that injects a SessionStart hook into every session -- the defect behind a week of degraded output on a field project. The fix is the owner's (disable the extra pack: `/plugin`), never the upgrade's: report it, do not edit `~/.claude`. The `commands`, `context`, and `roster` blocks gain their matching sentences (one-copy rule, agent memory holds preferences not state, the checkpoint budget is a floor) through the Phase 3-B marker graft.
...
*Language-independent CI delta (since v4.1.0):* the `docs-budget` job's caps were LOWERED, it gained a `docs/archive/` total check, and it now caps each `docs/plans/*.md` (the `iteration` skill's old "fit in half a context window" was the licence behind 30K-char plans). Report any existing plan over the cap; it belongs to an unfinished feature, so compact it rather than deleting it. A project on v4.0.0 has the old caps hard-coded in its `qa.yml`; replace that job's cap lines and error message with the fetched template's (this is the one case where the job is updated rather than only added, because the old numbers are the thing being fixed -- show the diff, as always). Replace that job's cap lines and error message with the fetched template's -- do not transcribe the numbers into this skill or into the project's prose. The job owns them; copying them here would be the same 'one fact written twice' defect this release exists to fix. Do not land this without Phase 3-F below, or the project's next push fails CI on docs it has not been told to compact.

*Language-independent delta (since v4.4.2) -- the dispatch economy:* ask the owner ONE question -- `weight: lite or full?` (lite for a solo/personal tool; full otherwise) -- then: add `"weight"` and `"model_tiers"` to `docs/agents.json` (Claude roster: `{"mechanical": "haiku", "standard": "sonnet", "judgment": "inherit"}`; other rosters: owner-filled TODOs). If lite, apply the template's `# FW-LITE-START/END`-fenced lines to `qa.yml` WITHOUT the markers (release-tag e2e trigger + job `if:`, advisory dup gate); if full, change nothing in `qa.yml` for this delta. Replace `.claude/skills/iteration/SKILL.md` and `scripts/factory_doctor.sh` with the fetched versions (both are template-owned procedure files, not hand-filled content -- but diff first and preserve any project-added lines by reporting them). Set `.claude/agents/reviewer.md` frontmatter `model:` to `inherit` (review is judgment-tier work; `sonnet` was the old pin). Run `bash scripts/factory_doctor.sh` once and put its new meter line (doc-maintenance commit ratio) in the Phase 4 report.

*Language-independent CI delta (since v4.4.2) -- mechanical doc death:* copy `scripts/prune.py` from the fetched template (placeholder-free, `chmod +x`) and append the `Entry shapes + leftovers a merge should have pruned` step (`python3 scripts/prune.py --check`) to the project's `docs-budget` job. This release also changes the ledger's semantics: the ledger holds OPEN features only -- `prune.py` deletes a closed feature's ledger and deviations lines, its plan, losing mockups, and uncited probes at merge, and the MERGED ledger line is no longer written (the merge commit is the record). `docs/proposals-ideas.md` is retired: if the project has one, promote anything still live to `features.json` todo entries and delete the file -- report the promoted count, never silently discard. **Run `python3 scripts/prune.py` once during the upgrade** (it clears the closed-feature backlog mechanically), then `python3 scripts/prune.py --check` and report what remains: over-cap entries are the owner's history and are NOT trimmed by the upgrade -- flag them in the report; the caps bind new entries going forward. When moving or trimming any entry, carry a status VERBATIM or delete it -- never reword an obligation into a claim (a real field bug: a compaction rewrote "live smoke mandatory" as "live smoke ran" while the work was still owed).
...
3. **Delete stale scaffolding**: every mockup in `docs/design/mockups/` that no
   ...
   about stays.
4. **Prune `docs/archive/`** to the total cap the `docs-budget` job sets, least-recently-cited first.
